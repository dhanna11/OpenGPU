...
entity soc_system_dbg_csig is
	generic (
		sld_auto_instance_index : string  := "YES";
		sld_instance_index      : integer := 0;
		instance_id             : string  := "CSIG";
		probe_width             : integer := 12;
		source_width            : integer := 0;
		enable_metastability    : string  := "NO"
	);
	port (
		probe : in std_logic_vector(11 downto 0) := (others => '0')  -- probes.probe
	);
end entity soc_system_dbg_csig;

architecture rtl of soc_system_dbg_csig is
...
begin
	...
	instance_id_check : if instance_id /= "CSIG" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	probe_width_check : if probe_width /= 12 generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;
	...
	dbg_csig : component altsource_probe_top
		generic map (
...
end architecture rtl; -- of soc_system_dbg_csig
